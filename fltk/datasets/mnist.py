...
class MNIST(Dataset):
    ...
    def load_train_dataset(self, rank: int = 0, world_size: int = None):
        train_dataset = datasets.MNIST(root=self.config.get_data_path(), train=True, download=True,
    ...
    def load_test_dataset(self):
        test_dataset = datasets.MNIST(root=self.config.get_data_path(), train=False, download=True,
                                      transform=self.DEFAULT_TRANSFORM)
...
class MNIST_RGB(Dataset):
    """
    MNIST Dataset implementation for Distributed learning experiments. Duplicate of channel to make RGB.
    """
    ...
    def __init__(self, config, learning_param, rank: int = 0, world_size: int = None):
        super(MNIST_RGB, self).__init__(config, learning_param, rank, world_size)
